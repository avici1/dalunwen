...
# -----------------------------------------------------------------------------
# 1. 路径与运行参数
# 默认仍指向原工作目录；可用环境变量改到本仓库 data/ 与 outputs/。
# -----------------------------------------------------------------------------
...
if (!dir.exists(dirname(default_out_dir)) && nzchar(Sys.getenv("CH5_ARTIFACT_DIR"))) {
  default_out_dir <- file.path(Sys.getenv("CH5_ARTIFACT_DIR"), "RSF_table5_3A")
}
