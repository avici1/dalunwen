...
# MCMC：n_chains = 3，用于计算 Gelman Rhat
...
exec_dir <- file.path(base_dir, "执行")
dir.create(exec_dir, showWarnings = FALSE, recursive = TRUE)
out_dir <- file.path(exec_dir, "图像_JM")
...
# 3 条链才能算 Gelman Rhat；Windows 上 JMbayes2 用 PSOCK 并行
n_chains_use <- 3L
n_iter_use <- 1500L
n_burnin_use <- 500L
...
# ---------------------------------------------------------------------------
# 纵向 lme + Cox + jm（已有 RDS 则跳过拟合）
# ---------------------------------------------------------------------------
jm_rds <- file.path(out_dir, "0827_JM_fit.rds")
if (file.exists(jm_rds)) {
...
flush.console()
train_pred <- pred_landmark(jmFit, train_long, train_surv, landmark_t, evaluation_horizon)
cat("[predict] landmark=", landmark_t, " 验证集...\n", sep = "")
flush.console()
test_pred <- pred_landmark(jmFit, test_long, test_surv, landmark_t, evaluation_horizon)
cat(
  "landmark 训练集 n =", nrow(train_pred),
  "| 死亡 =", sum(train_pred$event), "\n"
)
cat(
  "landmark 验证集 n =", nrow(test_pred),
  "| 死亡 =", sum(test_pred$event), "\n"
)
flush.console()
...
cat(sprintf(
  "4 IBS (landmark 至 28 天, IPCW) | 训练=%.4f | 测试=%.4f\n",
...
perf_word <- data.frame(
  ...
  events = c(sum(train_pred$event), sum(test_pred$event)),
  cindex = round(c(train_cindex, test_cindex), 4),
  brier = round(c(train_score_vals$brier, test_score_vals$brier), 4),
  stringsAsFactors = FALSE
)
