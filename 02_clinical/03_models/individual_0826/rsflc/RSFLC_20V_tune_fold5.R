...
repo_data_dir <- Sys.getenv("CH5_DATA_DIR", unset = "")
if (!dir.exists(default_base_dir) && nzchar(repo_data_dir) && dir.exists(repo_data_dir)) {
  default_base_dir <- repo_data_dir
}
base_dir <- Sys.getenv("RSFLC_INPUT_DIR", unset = default_base_dir)
default_out_dir <- file.path(base_dir, "5折RSFLC")
if (!dir.exists(base_dir) && nzchar(Sys.getenv("CH5_ARTIFACT_DIR"))) {
  default_out_dir <- file.path(Sys.getenv("CH5_ARTIFACT_DIR"), "RSFLC_20V_fivefold")
}
out_dir <- Sys.getenv("RSFLC_OUTPUT_DIR", unset = default_out_dir)
