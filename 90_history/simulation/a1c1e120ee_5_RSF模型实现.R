...
dat <- sim1000_70_10V_highINTER_3c[[1]]
...
#############KIMI####
dat <- as.data.frame(sim1000_70_10V_highINTER_3c[[1]])

## 2. 预测变量 ----------------------------------------------------------
xvars <- grep("^V[0-9]", names(dat), value = TRUE)   # 还是那 10 个变量

## 3. 一次性构造合法公式 ------------------------------------------------
form <- as.formula(
  paste("Surv(obs_time, event) ~", paste(xvars, collapse = " + "))
)

## 4. 拟合 ---------------------------------------------------------------
set.seed(123)
rfsrc_fit <- rfsrc(form,
                   data       = dat,
                   ntree      = 1000,
                   mtry       = floor(length(xvars)/3),
                   nodesize   = 10,
                   importance = TRUE,
                   proximity  = FALSE)
