...
# ---------------------------------------------------------------------------
# 开关
# RUN_TUNING：TRUE 跑 36 组网格；FALSE 则直接用下方 best_* 重训作图
# RUN_OOB：每组调用 compute_ooberror（较慢，失败则记 NA）
# RUN_SHAP：作 SHAP 三图（只扰动 11 个固定协变量，GCS 轨迹保持原值）
...
# Windows 上 ncores>1 会走 PSOCK，worker 找不到 seed_value；先固定单核
ncores_use <- 8L
...
result_path <- file.path(base_dir, "results_RSFLC.csv")
...
# do.call 先在主进程把参数求成字面值，避免 worker 去找 seed_value
...
extract_vimp_table <- function(vimp_obj) {
  imp <- vimp_obj$Importance
  if (is.null(imp)) imp <- vimp_obj$importance
  input_names <- vimp_obj$Inputs
  rows <- list()
  add_part <- function(vec, group) {
    if (is.null(vec) || length(vec) == 0) return(invisible(NULL))
    nm <- NULL
    if (!is.null(input_names[[group]])) {
      nm <- as.character(input_names[[group]])
    }
    if (is.null(nm) || length(nm) != length(vec) || any(!nzchar(nm))) {
      nm <- names(vec)
    }
    if (is.null(nm) || length(nm) != length(vec) || any(!nzchar(nm))) {
      nm <- paste0(group, "_", seq_along(vec))
    }
    rows[[length(rows) + 1]] <<- data.frame(
      variable = as.character(nm),
      importance = as.numeric(vec),
      group = group,
      stringsAsFactors = FALSE
    )
    invisible(NULL)
  ...
  if (is.list(imp)) {
    add_part(imp$Longitudinal, "Longitudinal")
    add_part(imp$Numeric, "Numeric")
    add_part(imp$Factor, "Factor")
  } else if (is.numeric(imp)) {
    add_part(imp, "Predictor")
  }
  if (length(rows) == 0) {
    stop("无法从 compute_vimp 结果中解析 Importance")
  }
  out <- dplyr::bind_rows(rows)
  out[order(-out$importance, na.last = TRUE), , drop = FALSE]
}

map_vimp_variable_names <- function(vimp_table, numeric_names, factor_names) {
  out <- vimp_table
  if (nrow(out) == 0) return(out)
  lon_idx <- which(out$group == "Longitudinal")
  num_idx <- which(out$group == "Numeric")
  fac_idx <- which(out$group == "Factor")
  if (length(lon_idx) >= 1L) {
    out$variable[lon_idx[1]] <- "gcs"
  }
  if (length(num_idx) > 0L && length(numeric_names) >= length(num_idx)) {
    out$variable[num_idx] <- numeric_names[seq_along(num_idx)]
  }
  if (length(fac_idx) > 0L && length(factor_names) >= length(fac_idx)) {
    out$variable[fac_idx] <- factor_names[seq_along(fac_idx)]
  }
  out
}
...
cat(
  "训练集 n =", nrow(train_pack$fixedData),
  "（group=1）| 测试集 n =", nrow(test_pack$fixedData),
  "（group=2）| ncores =", ncores_use, "\n"
)
flush.console()
...
p_mtry <- length(fixed_covars) + 1L
cat("预测变量数 p =", p_mtry, "（1 个 GCS +", length(fixed_covars), "个固定）\n")
flush.console()

# ---------------------------------------------------------------------------
# 网格调参
# ntree：包默认 200，vignette/已跑通脚本常用 50
# mtry：文献要求在 1…p 中选，默认 sqrt(p)≈3.5
# nodesize：默认 1；方法文常用 3；已跑通脚本用 5
# nsplit_option 固定 quantile；minsplit 仅生存结局，不搜
# ---------------------------------------------------------------------------
...
if (isTRUE(RUN_TUNING)) {
  ...
  if (all(is.na(tuning_results$testci))) {
    warning("全部组合 testci 均为 NA，沿用默认 best_*")
  } else {
    ...
    cat(sprintf(
      "按测试集 C-index 选定 combo %d：ntree=%d mtry=%d nodesize=%d\n",
  ...
  cat("跳过网格调参，使用 best_ntree/best_mtry/best_nodesize 重训\n")
  flush.console()
}
