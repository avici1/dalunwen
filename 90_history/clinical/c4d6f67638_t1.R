...
# 该 xlsx 含多个 sheet（每个 sheet = 一次模拟），需读成 list of data.frame
# 直接 read_xlsx() 只会读第 1 个 sheet；且 data.frame 在 R 里也是 list，
# circle_cal_3 会按“列”循环，导致 data_list[[i]] 变成向量而非数据框。
...
circle_cal_3 <- function(data_list, t0 = 1, verbose = TRUE) {
  ...
  # 检查输入（data.frame 在 R 中也是 list，必须先排除）
...
# 建议先用少量 sheet 试跑，确认无误后再跑全部 200 次：
# t2 <- circle_cal_3(t1[1:2], t0 = t0)
t2 <- circle_cal_3(t1, t0 = t0)
